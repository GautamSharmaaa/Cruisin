...
export function OrderManager({ orders, isLoading }: OrderManagerProps): ReactNode {
  ...
  return <section className="grid min-w-0 gap-6">
    <AdminStatsGrid className="xl:grid-cols-6">
      ...
      <AdminStat label="Pending" value={stats.pending} tone="warning" />
...
function OrderDrawer({ order, onClose }: { order: OrderDto; onClose: () => void }): ReactNode {
  return <div className="fixed inset-0 z-50 bg-background-primary/70 backdrop-blur" role="dialog" aria-modal="true">
    <div className="ml-auto grid h-full w-full max-w-3xl grid-rows-[auto_1fr] border-l border-border bg-background-primary shadow-lg">
      <div className="flex items-start justify-between gap-4 border-b border-border p-5"><div><p className="font-mono text-[11px] uppercase tracking-[0.16em] text-accent-gold">Order Detail</p><h2 className="mt-2 font-display text-2xl text-text-primary">{orderId(order)}</h2></div><button type="button" aria-label="Close order detail" onClick={onClose} className="grid h-10 w-10 place-items-center border border-border text-text-secondary transition hover:border-accent-gold hover:text-accent-gold"><X size={16} /></button></div>
      <div className="min-h-0 overflow-auto p-5">
        ...
        <AdminCard className="mt-4"><h3 className="text-text-primary">Items</h3><div className="mt-4 grid gap-3">{order.items?.map((item) => <div key={item.sku + item.title} className="grid gap-3 border border-border bg-background-elevated p-3 sm:grid-cols-[56px_1fr_auto]"><img src={item.image} alt="" className="h-14 w-14 object-cover" /><div><p className="text-sm text-text-primary">{item.title}</p><p className="font-mono text-xs text-text-muted">{item.sku}</p></div><p className="font-mono text-sm text-accent-gold">{item.quantity} x {formatPrice(item.price)}</p></div>)}</div></AdminCard>
