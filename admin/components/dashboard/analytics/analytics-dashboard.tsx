...
export function AnalyticsDashboard(): ReactNode {
  ...
  return <section className="grid w-full max-w-full min-w-0 grid-cols-[minmax(0,1fr)] gap-6">
    ...
    {analytics.isLoading ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5" aria-busy="true" aria-label="Loading analytics">{Array.from({ length: 10 }, (_unused, index) => <div key={index} className="h-36 animate-pulse border border-border bg-background-elevated" />)}</div> : null}
