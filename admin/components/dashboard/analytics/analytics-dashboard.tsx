...
export function AnalyticsDashboard(): ReactNode {
  ...
  const metrics: Metric[] = data ? [
    ...
    { label: 'COD collected', value: data.summary.codCollectedRevenue, previous: data.comparison.summary.codCollectedRevenue, format: 'currency', description: `Cash collected from ${data.summary.codCollectedOrders} delivered COD orders.` },
    { label: 'COD pending', value: data.summary.codPendingAmount, previous: data.comparison.summary.codPendingAmount, format: 'currency', description: `Cash still due from ${data.summary.codPendingOrders} undelivered COD orders.`, inverse: true },
    ...
    {data && tables ? <>
      ...
      <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,0.8fr)]">
        ...
        <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-6">
          <ChartShell title="Payment status" rows={statusRows(data.paymentStatus).rows} summary={`${data.summary.paidOrders} collected orders; ${data.summary.codCollectedOrders} COD collected; ${data.summary.codPendingOrders} COD pending; ${data.summary.refundedOrders} refunded.`}><div className="h-44"><ResponsiveContainer width="100%" height="100%"><BarChart data={Object.entries(data.paymentStatus).map(([label, value]) => ({ label: label.replaceAll('_', ' '), value }))}><XAxis dataKey="label" stroke={palette.muted} tickLine={false} tick={{ fontSize: 10 }} /><YAxis stroke={palette.muted} allowDecimals={false} /><Tooltip content={<AnalyticsTooltip />} /><Bar dataKey="value" name="Orders" fill={palette.cyan} /></BarChart></ResponsiveContainer></div></ChartShell>
