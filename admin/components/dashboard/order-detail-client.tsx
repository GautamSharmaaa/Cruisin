...
export function OrderDetailClient({ id }: OrderDetailClientProps): ReactNode {
  ...
  const requestRefund = (): void => {
    ...
    const fingerprint = `${amount}|${refundReason}`;
    if (refundAttempt.current?.fingerprint !== fingerprint) refundAttempt.current = { fingerprint, key: crypto.randomUUID() };
    paymentAction.mutate({ id: displayId, action: 'refund', amount, reason: refundReason, idempotencyKey: refundAttempt.current.key });
  };
  ...
  return <section className="grid gap-6">
    ...
    <div className="grid gap-6 lg:grid-cols-3"><article className="border border-border bg-background-elevated p-6"><h2 className="font-display text-xl">Payment</h2><p className="mt-4 text-sm text-text-secondary">{current.paymentMode ?? 'online'} · {current.paymentStatus}</p><p className="mt-2 font-mono text-accent-gold">Paid {formatPrice(current.amountPaid ?? 0)} · Refunded {formatPrice(current.refundAmount ?? 0)} · Due {formatPrice(current.amountDue ?? current.total)}</p><p className="mt-2 break-all text-xs text-text-muted">Razorpay order: {current.razorpayOrderId ?? '—'}<br />Payment: {current.razorpayPaymentId ?? '—'}</p></article><article className="border border-border bg-background-elevated p-6"><h2 className="font-display text-xl">{COPY.orders.shipping}</h2><p className="mt-4 text-sm text-text-secondary">{current.shippingAddress?.fullName ?? COPY.common.none}</p><p className="mt-2 text-sm text-text-secondary">{line([current.shippingAddress?.line1, current.shippingAddress?.line2, current.shippingAddress?.city, current.shippingAddress?.state, current.shippingAddress?.postalCode, current.shippingAddress?.country])}</p></article><article className="border border-border bg-background-elevated p-6"><h2 className="font-display text-xl">Payment operations</h2><div className="mt-4 grid gap-2">{current.paymentMode === 'cod' && current.paymentStatus !== 'paid' ? <Button onClick={() => paymentAction.mutate({ id: displayId, action: 'mark-cod-paid' })} disabled={paymentAction.isPending}>Mark COD paid</Button> : null}{current.paymentMode === 'partial' && current.amountDue ? <Button onClick={() => paymentAction.mutate({ id: displayId, action: 'mark-partial-paid' })} disabled={paymentAction.isPending}>Mark remaining collected</Button> : null}{canRefund ? <><Input label="Refund amount" type="number" min="1" max={String((current.amountPaid ?? 0) - (current.refundAmount ?? 0))} value={refundAmount} onChange={(event) => setRefundAmount(event.target.value)} /><Input label="Refund reason" value={refundReason} onChange={(event) => setRefundReason(event.target.value)} /><Button variant="secondary" onClick={requestRefund} disabled={!Number(refundAmount) || paymentAction.isPending}>Refund paid amount</Button></> : null}</div></article></div>
    {(current.refunds ?? []).length ? <article className="border border-border bg-background-elevated p-6"><h2 className="font-display text-xl">Refund history</h2><div className="mt-4 grid gap-3">{current.refunds?.map((refund) => <div key={refund.providerRefundId ?? `${refund.amount}-${refund.createdAt}`} className="grid gap-1 border-l border-accent-gold pl-4 text-sm text-text-secondary"><p className="font-mono text-text-primary">{refund.providerRefundId ?? 'Provider refund pending'}</p><p>{formatPrice(refund.amount)} · {refund.status}</p>{refund.reason ? <p>{refund.reason}</p> : null}</div>)}</div></article> : null}
    <div className="overflow-x-auto border border-border bg-background-elevated"><table className="w-full min-w-[820px] text-left text-sm"><thead className="text-xs uppercase tracking-[0.12em] text-text-secondary"><tr><th className="border-b border-border p-4">Item</th><th className="border-b border-border p-4">Variant</th><th className="border-b border-border p-4">SKU</th><th className="border-b border-border p-4">Quantity</th><th className="border-b border-border p-4">Price</th></tr></thead><tbody>{(current.items ?? []).map((item) => <tr key={item.sku + item.title} className="border-b border-border-subtle"><td className="p-4 text-text-primary">{item.title}</td><td className="p-4 text-text-secondary">{[item.color, item.size].filter(Boolean).join(' / ') || 'Legacy order'}</td><td className="p-4 font-mono text-text-secondary">{item.sku}</td><td className="p-4 text-text-secondary">{item.quantity}</td><td className="p-4 font-mono text-accent-gold">{formatPrice(item.price)}</td></tr>)}</tbody></table></div>
