...
export function CategoryManager({ categories, isLoading }: CategoryManagerProps): ReactNode {
  ...
  const onSubmit = (data: CategoryFormValues): void => {
    if (selectedCategory) {
      updateCategory.mutate({ ...data, id: categoryId(selectedCategory) }, { onSuccess: () => setEditingId(null) });
      return;
    }
    createCategory.mutate(data, { onSuccess: () => reset(emptyCategoryValues) });
  };
  ...
  const onToggle = (category: CategoryDto): void => {
    updateCategory.mutate({ ...categoryValues(category), id: categoryId(category), isActive: !category.isActive });
  };
    ...
    <div className="grid gap-6 xl:grid-cols-[420px_1fr]">
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 border border-border bg-background-elevated p-5 shadow-lg">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
          <Input label={COPY.fields.name} error={formState.errors.name?.message} {...register('name', { onBlur: () => setValue('slug', slugify(name ?? '')) })} />
          ...
          <Input label="Description" error={formState.errors.description?.message} {...register('description')} />
          ...
          <Input label="Canonical Slug" error={formState.errors.canonicalSlug?.message} {...register('canonicalSlug')} />
        ...
        {createCategory.isSuccess && mode === 'create' ? <p className="text-sm text-success"><Check size={15} className="mr-2 inline" />{COPY.categories.created}</p> : null}
        {updateCategory.isSuccess ? <p className="text-sm text-success"><Check size={15} className="mr-2 inline" />Category updated.</p> : null}
      </form>
