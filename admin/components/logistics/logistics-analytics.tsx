...
export function LogisticsAnalytics(): ReactNode {
  ...
  return <div className="grid gap-6">
    ...
    <section className="grid gap-6 xl:grid-cols-2">
      <article className="border border-border bg-background-elevated p-5"><h2 className="font-display text-2xl">Daily shipment cost</h2><div className="mt-5 grid gap-3">{analytics.data?.daily.map((row) => <div key={row._id} className="grid grid-cols-[100px_1fr_auto] items-center gap-3 text-xs"><span className="text-text-muted">{row._id}</span><span className="h-2 bg-accent-gold/30"><span className="block h-full bg-accent-gold" style={{ width: `${Math.min(100, row.shipments * 10)}%` }} /></span><span className="font-mono">{row.shipments} · {formatPrice(row.cost)}</span></div>)}</div></article>
      <article className="border border-border bg-background-elevated p-5"><h2 className="font-display text-2xl">Courier scorecard</h2><div className="mt-5 overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left text-xs uppercase text-text-muted"><th className="pb-3">Courier</th><th className="pb-3">Shipments</th><th className="pb-3">Delivered</th><th className="pb-3">NDR</th><th className="pb-3">Cost</th></tr></thead><tbody>{analytics.data?.couriers.map((row) => <tr key={row._id} className="border-t border-border-subtle"><td className="py-3">{row._id}</td><td>{row.shipments}</td><td>{row.delivered}</td><td>{row.ndr}</td><td>{formatPrice(row.cost)}</td></tr>)}</tbody></table></div></article>
    </section>
