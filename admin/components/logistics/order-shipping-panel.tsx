...
export function OrderShippingPanel({ orderId }: { orderId: string }): ReactNode {
  ...
  return <article className="border border-border bg-background-elevated p-6">
    ...
    <p className="mt-2 text-sm text-text-secondary">Create the Shiprocket order here. Complete courier, AWB, documents and pickup manually in Shiprocket; Cruisin mirrors the result.</p>
    <div className="mt-5 grid gap-5">
      {shipments.data?.items.map((shipment) => <section key={shipment._id} className="border-l border-accent-gold pl-4">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-xs text-accent-gold">{shipment.sourceOrderId}</p><h3 className="mt-1 text-lg text-text-primary">{label(shipment.shipmentStatus)}</h3></div>{shipment.providerOrderId || shipment.providerShipmentId ? <div className="flex flex-wrap gap-2"><Button className="min-h-9 px-4" onClick={() => setShipDialog(shipment)}>Ship <ExternalLink className="ml-2 h-3 w-3" /></Button><Button variant="secondary" className="min-h-9 px-4" onClick={() => runAction(`/admin/logistics/${shipment._id}/sync`, 'Synced just now')} disabled={action.isPending}><RefreshCw className={`mr-2 h-3 w-3 ${action.isPending ? 'animate-spin' : ''}`} />Sync now</Button></div> : null}</div>
        ...
        <p className="mt-4 text-xs text-text-muted">Customer delivery {shipment.shippingChargeCollected === undefined ? 'Unavailable' : formatPrice(shipment.shippingChargeCollected)} · Courier freight {shipment.providerShippingCost === undefined ? 'Unavailable' : formatPrice(shipment.providerShippingCost)}</p>
      </section>)}
    </div>
    ...
    {!shipments.isLoading && !providerShipment ? <Button className="mt-5" onClick={() => runAction(`/admin/logistics/orders/${orderId}/create`, 'Shiprocket provider order created successfully.')} disabled={action.isPending}>{action.isPending ? 'Creating Shiprocket order…' : 'Create Shiprocket Order'}</Button> : null}
