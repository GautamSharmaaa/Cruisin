...
const actionLabel = (request: WorkflowRequest, action: string): string => {
  if (request.refundPaymentMode === 'razorpay_original' && action === 'refund_pending') return 'Issue Razorpay refund';
  if (request.refundPaymentMode === 'razorpay_original' && action === 'refunded') return 'Sync Razorpay refund status';
  return returnWorkflowActionLabel(action);
};
    ...
    {requests.data?.map((request) => <article key={request._id} className="grid gap-4 border border-border bg-background-elevated p-5 lg:grid-cols-[1fr_auto] lg:items-start">
      <div>
        ...
        <dl className="mt-4 grid gap-2 text-xs text-text-muted sm:grid-cols-2">
          ...
          {request.providerRefund ? <div><dt>Razorpay provider status</dt><dd className="mt-1 text-text-primary">{request.providerRefund.status.replaceAll('_', ' ')} · ₹{request.providerRefund.amount}{request.providerRefund.createdAt ? ` · ${new Date(request.providerRefund.createdAt).toLocaleString('en-IN')}` : ''}</dd></div> : null}
      ...
      <div className="grid min-w-64 gap-2">
        ...
        <div className="flex flex-wrap gap-2">{visibleReturnWorkflowActions(request.status, me.data?.role, request.allowedActions).map((value) => <Button key={value} variant={value.includes('reject') ? 'secondary' : 'primary'} onClick={() => { if (value === 'refund_pending' && !window.confirm(`Initiate ${request.productRefundAmount ?? 0} INR ${request.refundPaymentMode === 'razorpay_original' ? 'Razorpay refund to the original payment method' : `refund to ${request.refundDestination?.manualUpiId ?? request.refundDestination?.maskedDetails ?? request.refundDestination?.method ?? 'the selected destination'}`}? This is a financial action.`)) return; action.mutate({ id: request._id, action: value }); }} disabled={action.isPending || (value === 'refund_pending' && request.refundDestination?.verificationStatus !== 'verified')}>
          {request.refundPaymentMode === 'razorpay_original' && value === 'refunded' ? <RefreshCw className="h-4 w-4" /> : null}{actionLabel(request, value)}
        </Button>)}</div>
      </div>
    </article>)}
