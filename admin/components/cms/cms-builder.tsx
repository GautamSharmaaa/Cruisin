...
function BannerRow({ banner }: BannerRowProps): ReactNode {
  ...
  return <button ref={(node) => { draggable.setNodeRef(node); droppable.setNodeRef(node); }} {...draggable.listeners} {...draggable.attributes} className="min-h-16 cursor-grab border border-border p-4 text-left active:cursor-grabbing"><span className="block font-display text-lg text-text-primary">{banner.title}</span><span className="mt-1 block text-xs uppercase tracking-[0.15em] text-text-secondary">{banner.position} / {banner.isActive ? COPY.table.active : COPY.table.inactive}</span></button>;
}
...
export function CmsBuilder(_props: CmsBuilderProps): ReactNode {
  ...
  return <DndContext onDragEnd={onDragEnd}><div className="grid gap-6 lg:grid-cols-[1fr_420px]"><section className="grid gap-6"><form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 border border-border bg-background-elevated p-6 md:grid-cols-2"><h1 className="font-display text-2xl md:col-span-2">{COPY.cms.form}</h1><Input label={COPY.fields.title} error={formState.errors.title?.message} {...register('title')} /><Input label={COPY.fields.subtitle} error={formState.errors.subtitle?.message} {...register('subtitle')} /><Input label={COPY.fields.ctaText} error={formState.errors.ctaText?.message} {...register('ctaText')} /><Input label={COPY.fields.ctaLink} error={formState.errors.ctaLink?.message} {...register('ctaLink')} /><Input label={COPY.fields.image} error={formState.errors.image?.message} {...register('image')} /><Input label={COPY.fields.mobileImage} error={formState.errors.mobileImage?.message} {...register('mobileImage')} /><Input label={COPY.fields.position} error={formState.errors.position?.message} {...register('position')} /><Input label={COPY.fields.sortOrder} type="number" error={formState.errors.sortOrder?.message} {...register('sortOrder')} /><Input label={COPY.fields.startDate} type="date" error={formState.errors.startDate?.message} {...register('startDate')} /><Input label={COPY.fields.endDate} type="date" error={formState.errors.endDate?.message} {...register('endDate')} /><SelectField label={COPY.fields.active} options={[{ label: COPY.common.yes, value: 'true' }, { label: COPY.common.no, value: 'false' }]} value={watch('isActive') ? 'true' : 'false'} onChange={(event) => setValue('isActive', event.target.value === 'true')} /><Button type="submit" disabled={createBanner.isPending}>{createBanner.isPending ? COPY.common.loading : COPY.common.save}</Button>{createBanner.isSuccess ? <p className="text-sm text-success md:col-span-2">{COPY.cms.created}</p> : null}</form><div className="border border-border bg-background-elevated p-6"><h2 className="font-display text-xl">{COPY.cms.reorder}</h2><div className="mt-6 grid gap-3">{banners.isLoading ? <p className="text-sm text-text-secondary">{COPY.common.loading}</p> : bannerList.map((banner) => <BannerRow key={bannerId(banner)} banner={banner} />)}</div></div></section><aside className="border border-border bg-background-elevated p-6"><h2 className="font-display text-xl">{COPY.cms.preview}</h2><div className="mt-6 aspect-[3/4] border border-border bg-background-primary p-6">{activePreview ? <div className="flex h-full flex-col justify-end bg-background-overlay p-6"><p className="text-xs uppercase tracking-[0.15em] text-accent-gold">{activePreview.position}</p><h3 className="mt-3 font-display text-3xl text-text-primary">{activePreview.title}</h3><p className="mt-3 text-sm text-text-secondary">{activePreview.subtitle}</p><p className="mt-6 text-xs uppercase tracking-[0.15em] text-text-primary">{activePreview.cta?.text}</p></div> : <p className="text-sm text-text-secondary">{COPY.cms.empty}</p>}</div></aside></div></DndContext>;
}
