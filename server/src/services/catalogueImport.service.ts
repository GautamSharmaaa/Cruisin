...
import {
  ...
  numberFromCell,
  slugify,
  type CatalogueColumnMapping,
} from "./catalogueMapper.service.js";
import {
  parseCatalogueCsv,
  type CatalogueProductGroup,
  type ParsedCatalogue,
} from "./catalogueParser.service.js";
...
import {
  ...
  sanitizeDescription,
  validateCatalogue,
} from "./catalogueValidator.service.js";

export type CatalogueImportMode =
  | "dry-run"
  | "create-only"
  | "update-only"
  | "upsert"
...
const imageObject = (url: string, alt: string): Record<string, unknown> => ({
  url,
  alt: alt || "Cruisin product image",
  width: 1200,
  height: 1600,
});
...
const existingProductForCode = async (
  productCode: string,
...
const ensureCollections = async (
  names: string[],
): Promise<{ ids: Types.ObjectId[]; slugs: string[]; created: number }> => {
  const ids: Types.ObjectId[] = [];
  const slugs: string[] = [];
  let created = 0;
...
const collectionsForGroup = (
  group: CatalogueProductGroup,
  overrides?: Record<string, string | "ignore">,
): string[] => {
  const mapped = group.collections
    .map((name) => overrides?.[name] ?? name)
    .filter((name) => name !== "ignore") as string[];
...
const buildProductPatch = async (
  ...
  const description = sanitizeDescription(
    inherited[mapping.description] || title,
  );
  const textDescription = plainText(description) || title;
  const variantImageUrls = Array.from(
    new Set(group.variants.flatMap((variant) => variant.images)),
  );
  const productImageUrls = group.images.length
    ? group.images
    : variantImageUrls;
  const primaryImage = productImageUrls[0] || placeholderImage;
  const images = (
    productImageUrls.length ? productImageUrls : [placeholderImage]
  ).map((url) => imageObject(url, title));
  const category = await ensureCategoryPath(
    categoryPathForGroup(group, options.categoryMapping),
  );
  const collections = await ensureCollections(
    collectionsForGroup(group, options.collectionMapping),
  );
  const basePrice =
    numberFromCell(inherited[mapping.sellingPrice]) ??
    group.variants.find((variant) => variant.price > 0)?.price ??
    0;
  const comparePrice =
    numberFromCell(inherited[mapping.mrp]) ??
    group.variants.find((variant) => variant.comparePrice !== undefined)
      ?.comparePrice;
  const visible = booleanFromCell(inherited[mapping.visibility]);
  const slug = await ensureUniqueSlug(
    slugify(title + "-" + group.productCode),
    productId,
  );
  return {
    title,
    ...
    tags: Array.from(
      new Set([
        ...categoryPathForGroup(group, options.categoryMapping),
        ...Object.values(group.attributes).slice(0, 8),
      ]),
    ).filter(Boolean),
    ...
    productHighlights: Object.entries(group.attributes)
      .slice(0, 6)
      .map(([key, value]) => key + ": " + value),
    seo: {
      metaTitle: title,
      metaDesc: textDescription.slice(0, 155),
...
const buildVariants = (
  ...
  for (const existing of existingVariants)
    nextBySku.set(String(existing.sku ?? "").toUpperCase(), existing);
    ...
    const next = {
      ...
      images: variant.images.length
        ? variant.images.map((url) =>
            imageObject(url, group.inherited.Name || group.productCode),
          )
        : (existing?.images ?? [
            imageObject(image, group.inherited.Name || group.productCode),
          ]),
    };
...
const summarize = async (
  ...
  return {
    ...
    variantsToCreate,
    variantsToUpdate,
    categoriesToReview: Array.from(
      new Set(
        parsed.groups.map(
          (group) =>
            group.categorySuggestion.raw +
            " -> " +
            group.categorySuggestion.path.join(" > "),
        ),
      ),
    ).filter(Boolean),
    collectionsToCreate: Array.from(
      new Set(parsed.groups.flatMap((group) => group.collections)),
    ),
  };
};
...
const assertImportableCatalogue = (parsed: ParsedCatalogue): void => {
  if (parsed.rows.length === 0)
    throw new ApiError(
      400,
      "Catalogue CSV must include at least one product row",
    );
  if (parsed.groups.length === 0)
    throw new ApiError(
      400,
      "Catalogue CSV must include at least one product code",
    );
};
...
export const CatalogueImportService = {
  async upload(options: CatalogueImportOptions): Promise<unknown> {
    ...
    return {
      ...
      parsed: this.previewPayload(parsed, validation, summary),
    };
  },
  ...
  previewPayload(
    parsed: ParsedCatalogue,
    validation: ReturnType<typeof validateCatalogue>,
    ...
    return {
      ...
      detectedCollections: Array.from(
        new Set(parsed.groups.flatMap((group) => group.collections)),
      ),
      validation,
      summary,
  ...
  async confirm(options: CatalogueImportOptions): Promise<unknown> {
    ...
    if (validation.errors.length > 0)
      throw new ApiError(400, "Fix catalogue errors before import");
    const importRecord = options.importId
      ? await CatalogueImportModel.findById(options.importId).select(
          "+originalFileData",
        )
      : null;
    if (importRecord)
      await importRecord.updateOne({
        status: "importing",
        startedAt: new Date(),
        importMode: options.importMode ?? "upsert",
      });
    ...
    try {
      for (const group of parsed.groups) {
        ...
        if (
          ...
          const variantBuild = buildVariants(
            group,
            existing.variants as unknown as Array<Record<string, unknown>>,
          );
          const partial: Record<string, unknown> = {
            lastCatalogueImportId: importRecord?._id ?? null,
          };
          if (mode === "stock-only")
            partial.variants = variantBuild.variants.map((variant) => {
              const incoming = group.variants.find(
                (item) =>
                  item.sku.toUpperCase() === String(variant.sku).toUpperCase(),
              );
          ...
          if (mode === "media-only")
            partial.images = mergedImages(
              (existing.images as unknown as Array<Record<string, unknown>>) ??
                [],
              patch.images as Array<Record<string, unknown>>,
              options.replaceImages,
            );
          if (mode === "taxonomy-only")
            Object.assign(partial, {
              category: patch.category,
              categoryIds: patch.categoryIds,
              collections: patch.collections,
              collectionSlugs: patch.collectionSlugs,
            });
          await ProductModel.findByIdAndUpdate(existing._id, partial, {
            runValidators: true,
          });
        ...
        const variantBuild = buildVariants(
          group,
          existing?.variants as unknown as
        ...
        const product = existing
          ? await ProductModel.findByIdAndUpdate(existing._id, payload, {
              new: true,
              runValidators: true,
            })
  ...
  async csvForOptions(options: CatalogueImportOptions): Promise<string> {
    ...
    if (!options.importId)
      throw new ApiError(400, "Import id or CSV content is required");
    const record = await CatalogueImportModel.findById(options.importId).select(
      "+originalFileData",
    );
    if (!record?.originalFileData)
      throw new ApiError(404, "Stored catalogue file not found");
  ...
  async errorReport(id: string): Promise<string> {
    ...
    return (
      record.errorReportData || "Severity,Row,Product Code,Field,Message\n"
    );
  },
};
