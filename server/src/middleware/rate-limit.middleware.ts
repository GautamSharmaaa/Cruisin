...
export const otpRequestLimiter = rateLimit({ windowMs: 60_000, limit: 5, standardHeaders: true, legacyHeaders: false, handler: (_req, res) => { res.status(429).json(new ApiResponse(null, 'Too many OTP requests', ['RATE_LIMITED'])); } });
export const otpVerifyLimiter = rateLimit({ windowMs: 60_000, limit: 5, standardHeaders: true, legacyHeaders: false, handler: (_req, res) => { res.status(429).json(new ApiResponse(null, 'Too many OTP verification attempts', ['RATE_LIMITED'])); } });
