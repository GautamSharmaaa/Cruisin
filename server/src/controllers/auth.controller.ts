...
export const AuthController = {
  ...
  updateMe: asyncHandler(async (req: Request, res: Response): Promise<void> => { if (!req.user) throw new ApiError(401, 'Authentication required'); const user = await AuthService.updateMe(req.user.userId, req.body as { name?: string; email?: string; phone?: string; avatar?: string }); res.json(new ApiResponse(user, 'Profile updated')); }),
  changePassword: asyncHandler(async (req: Request, res: Response): Promise<void> => { if (!req.user) throw new ApiError(401, 'Authentication required'); await AuthService.changePassword(req.user.userId, req.body as { currentPassword: string; password: string }); res.json(new ApiResponse(null, 'Password updated')); }),
  addAddress: asyncHandler(async (req: Request, res: Response): Promise<void> => { if (!req.user) throw new ApiError(401, 'Authentication required'); const addresses = await AuthService.addAddress(req.user.userId, req.body as Record<string, unknown>); res.status(201).json(new ApiResponse(addresses, 'Address added')); }),
