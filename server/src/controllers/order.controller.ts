...
export const OrderController = {
  ...
  cod: asyncHandler(async (req: Request<Record<string, string>, unknown, { shippingAddress: Record<string, unknown>; billingAddress: Record<string, unknown>; shippingMethod?: 'standard' | 'express'; logisticsQuoteId?: string; couponCode?: string; idempotencyKey: string }>, res: Response): Promise<void> => { const result = await OrderService.createCodOrder(req.user?.userId ?? '', { ...req.body, paymentMethod: 'cod', paymentMode: 'cod' }); res.status(201).json(new ApiResponse(result, 'COD order placed')); }),
