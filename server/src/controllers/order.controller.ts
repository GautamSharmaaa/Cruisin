...
export const OrderController = {
  checkout: asyncHandler(async (req: Request<Record<string, string>, unknown, { shippingAddress: Record<string, unknown>; billingAddress: Record<string, unknown>; paymentMethod: PaymentMethod; paymentMode?: CheckoutPaymentMode; shippingMethod?: 'standard' | 'express'; logisticsQuoteId?: string; couponCode?: string; idempotencyKey: string; metaEventId?: string }>, res: Response): Promise<void> => { const result = await OrderService.checkout(req.user?.userId ?? '', req.body); res.status(201).json(new ApiResponse(result, 'Checkout initialized')); }),
  ...
  customerCancel: asyncHandler(async (req: Request<Record<string, string>, unknown, { reasonCode: 'changed_mind' | 'wrong_item' | 'delivery_too_slow' | 'found_better_option' | 'other'; details?: string }>, res: Response): Promise<void> => { const order = await OrderService.cancelByCustomer(String(req.params.id ?? ''), req.user?.userId ?? '', req.body); res.json(new ApiResponse(order, 'Order cancelled')); }),
  ...
  cod: asyncHandler(async (req: Request<Record<string, string>, unknown, { shippingAddress: Record<string, unknown>; billingAddress: Record<string, unknown>; shippingMethod?: 'standard' | 'express'; logisticsQuoteId?: string; couponCode?: string; idempotencyKey: string; metaEventId?: string }>, res: Response): Promise<void> => { const result = await OrderService.createCodOrder(req.user?.userId ?? '', { ...req.body, paymentMethod: 'cod', paymentMode: 'cod' }); res.status(201).json(new ApiResponse(result, 'COD order placed')); }),
  ...
  syncRefund: asyncHandler(async (req: Request, res: Response): Promise<void> => { const order = await OrderService.syncLatestRefund(String(req.params.id ?? '')); res.json(new ApiResponse(order, 'Refund status synchronized')); })
};
