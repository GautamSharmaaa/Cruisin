...
export const CartController = {
  ...
  applyCoupon: asyncHandler(async (req: Request<Record<string, string>, unknown, { code: string }>, res: Response): Promise<void> => { const result = await CartService.applyCoupon(req.user?.userId, req.sessionId, req.body.code); res.json(new ApiResponse(result, 'Coupon applied')); })
};
