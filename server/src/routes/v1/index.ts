// Governed by .rules v1.0
import { Router } from 'express';
import { adminRouter } from './admin.routes.js';
import { adminManagementRouter } from './admin-management.routes.js';
import { authRouter } from './auth.routes.js';
import { cartRouter } from './cart.routes.js';
import { categoryRouter } from './category.routes.js';
import { cmsRouter } from './cms.routes.js';
import { orderRouter } from './order.routes.js';
import { paymentRouter } from './payment.routes.js';
import { notificationRouter } from './notification.routes.js';
import { newsletterRouter } from './newsletter.routes.js';
import { collectionRouter, merchandisingAdminRouter, navigationRouter, pageSettingsRouter, siteSettingsRouter, tagRouter } from './merchandising.routes.js';
import { productRouter } from './product.routes.js';
import { reviewRouter } from './review.routes.js';
import { wishlistRouter } from './wishlist.routes.js';
import { adminLogisticsRouter, logisticsRouter } from './logistics.routes.js';
import { logisticsWebhookRouter } from './logistics-webhook.routes.js';
import { adminExchangeRouter, adminReturnRouter, returnExchangeRouter } from './return-exchange.routes.js';
import { promotionExperienceAdminRouter, promotionExperienceRouter } from './promotion-experience.routes.js';

export const v1Router = Router();
v1Router.use('/auth', authRouter);
v1Router.use('/products', productRouter);
v1Router.use('/navigation', navigationRouter);
v1Router.use('/collections', collectionRouter);
v1Router.use('/page-settings', pageSettingsRouter);
v1Router.use('/site-settings', siteSettingsRouter);
v1Router.use('/promotion-experience', promotionExperienceRouter);
v1Router.use('/tags', tagRouter);
v1Router.use('/categories', categoryRouter);
v1Router.use('/cart', cartRouter);
v1Router.use('/orders', orderRouter);
v1Router.use('/logistics', logisticsRouter);
v1Router.use('/webhooks', logisticsWebhookRouter);
v1Router.use('/fulfillment', returnExchangeRouter);
v1Router.use('/payments', paymentRouter);
v1Router.use('/wishlist', wishlistRouter);
v1Router.use('/notifications', notificationRouter);
v1Router.use('/newsletter', newsletterRouter);
v1Router.use('/cms', cmsRouter);
v1Router.use('/reviews', reviewRouter);
v1Router.use('/admin', adminRouter);
v1Router.use('/admin', merchandisingAdminRouter);
v1Router.use('/admin', adminManagementRouter);
v1Router.use('/admin/promotion-experience', promotionExperienceAdminRouter);
v1Router.use('/admin/logistics', adminLogisticsRouter);
v1Router.use('/admin/returns', adminReturnRouter);
v1Router.use('/admin/exchanges', adminExchangeRouter);
