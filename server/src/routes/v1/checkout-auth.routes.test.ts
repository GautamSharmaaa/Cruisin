...
const { orderController, paymentController } = vi.hoisted(() => {
  ...
  return {
    orderController: { checkout: vi.fn(noop), cod: vi.fn(noop), verify: vi.fn(noop), paymentFailed: vi.fn(noop), paymentCancelled: vi.fn(noop), mine: vi.fn(noop), all: vi.fn(noop), byId: vi.fn(noop), updateStatus: vi.fn(noop), customerCancel: vi.fn(noop), syncRefund: vi.fn(noop), paymentStatus: vi.fn(noop), markCodPaid: vi.fn(noop), markPartialPaid: vi.fn(noop) },
...
describe('checkout and order access', () => {
  const anonymousRequests = [
    ...
    ['post', '/payments/razorpay/payment-cancelled'],
    ['get', '/orders/mine'],
