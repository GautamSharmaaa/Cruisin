...
export const invoiceListQuerySchema = invoiceListQueryBaseSchema.superRefine((value, context) => {
  if (value.startDate && value.endDate && value.startDate > value.endDate)
    context.addIssue({
      ...
      path: ['endDate'],
      message: 'Invoice end date must be on or after start date',
    });
  if (value.orderStartDate && value.orderEndDate && value.orderStartDate > value.orderEndDate)
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['orderEndDate'],
...
export const bulkInvoicePdfSchema = z
  .object({
  ...
  .superRefine((value, context) => {
    if (!value.selectAll && (!value.invoiceIds || value.invoiceIds.length === 0))
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['invoiceIds'],
        message: 'Select at least one invoice',
      });
    if (value.selectAll && value.invoiceIds?.length)
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['invoiceIds'],
        message: 'Use either selected invoice ids or all filtered results',
      });
    if (value.invoiceIds && new Set(value.invoiceIds).size !== value.invoiceIds.length)
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['invoiceIds'],
        message: 'Invoice selection contains duplicates',
      });
  });
...
export const invoiceSettingsSchema = z.object({
  ...
  invoicePrefix: z
    .string()
    .trim()
    .regex(/^[A-Za-z0-9-]{1,12}$/, 'Use letters, numbers, or hyphens only')
    .transform((value) => value.toUpperCase()),
  registeredAddress: z.string().trim().max(500),
  gstin: z
    .string()
    .trim()
    .regex(/^$|^[0-9]{2}[A-Z0-9]{13}$/, 'Enter a valid 15-character GSTIN')
    .transform((value) => value.toUpperCase()),
  state: z.string().trim().max(80),
  stateCode: z
    .string()
    .trim()
    .regex(/^$|^[0-9]{2}$/, 'State code must contain two digits'),
