...
describe("product validator ordered variant media", () => {
  ...
  it("enforces the fixed 5% GST-inclusive product rate", () => {
    expect(
      productBodySchema.safeParse({ ...product, gstPercent: 5 }).success,
    ).toBe(true);
    expect(
      productBodySchema.safeParse({ ...product, gstPercent: 18 }).success,
    ).toBe(false);
  });
  ...
  it("requires MRP to be greater than the selling price", () => {
    expect(
      productBodySchema.safeParse({ ...product, comparePrice: 1_999 }).success,
    ).toBe(true);
    expect(
      productBodySchema.safeParse({ ...product, comparePrice: 799 }).success,
    ).toBe(false);
    expect(
      productBodySchema.safeParse({ ...product, comparePrice: 699 }).success,
    ).toBe(false);
  });
