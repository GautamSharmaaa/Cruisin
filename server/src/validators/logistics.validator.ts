...
export const documentParamSchema = z.object({ shipmentId: objectIdSchema, kind: z.enum(['label', 'invoice', 'manifest']) });
...
export const packageConfirmationSchema = z.object({
  ...
  if (Math.abs(value.deadWeightKg - (value.productWeightKg + value.packagingWeightKg)) > 0.01) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['deadWeightKg'], message: 'Dead weight must equal product plus packaging weight' });
  }
});

export const courierSelectionSchema = z.object({ courierId: z.number().int().positive().optional() }).strict();
export const ndrActionSchema = z.object({
  action: z.enum(['reattempt', 'correct_address', 'confirm_availability', 'update_phone', 'contacted', 'escalate', 'accept_rto', 'note']),
  note: z.string().trim().max(500).optional()
}).strict();
export const rtoWarehouseSchema = z.object({ action: z.enum(['received', 'inspection_passed', 'inspection_failed']) }).strict();

const providerTimestampSchema = z.string().trim().max(80).refine((value) => !Number.isNaN(Date.parse(value)), 'Invalid provider timestamp');
...
export const logisticsWebhookSchema = z.object({
  ...
  if (!value.awb && !value.awb_code && !value.order_id && !value.shipment_id) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: 'Webhook requires a shipment identifier' });
  }
  if (!value.current_status && !value.status) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: 'Webhook requires a shipment status' });
  }
});
