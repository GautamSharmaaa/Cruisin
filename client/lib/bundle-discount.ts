...
export const recommendationBundleDiscount = (lines: BundleCartLine[], recommendation?: RecommendationBundlePricing): number | undefined => {
  ...
  if (!recommendation.bundleDiscount.enabled) return 0;
  const cartIds = new Set(lines.filter((line) => line.quantity > 0).map((line) => line.product.id));
  const eligibleIds = recommendation.source === 'manual'
    ? new Set([recommendation.anchorProductId ?? '', ...recommendation.eligibleProductIds])
    : cartIds;
...
export const automaticBundleDiscount = (lines: BundleCartLine[]): AutomaticBundleDiscount => {
  ...
  for (const line of visibleLines) {
    ...
    if (config?.enabled === false || config?.bundleDiscount?.enabled !== true) continue;
    const eligibleIds = config.strategy === 'manual'
      ? new Set([line.product.id, ...(line.product.recommendedProductIds ?? [])])
      : cartIds;
    ...
    const twoItemDiscount = Math.min(TWO_ITEM_DISCOUNT_CAP, money(config.bundleDiscount.twoItemDiscount ?? 0));
    const threeItemDiscount = Math.min(THREE_ITEM_DISCOUNT_CAP, money(config.bundleDiscount.threeItemDiscount ?? 0));
