...
export default function CheckoutPage(): ReactNode {
  ...
  const orderTotal = useCartStore((state) => {
    const subtotal = state.items.filter((item) => isCustomerVisibleProduct(item.product)).reduce((sum, item) => sum + item.price * item.quantity, 0);
    const discountedSubtotal = Math.max(0, subtotal - state.couponDiscount);
    const shipping = state.freeShipping || discountedSubtotal >= 25000 ? 0 : 900;
    return discountedSubtotal + shipping + Math.round(discountedSubtotal * 0.18);
  });
  ...
  const finish = (order: { _id?: string; id?: string; orderNumber?: string; paymentMode?: string; amountPaid?: number; amountDue?: number }, state: 'success' | 'failure'): void => { const id = getOrderId(order); if (state === 'success') useCartStore.getState().clearCart(); const query = new URLSearchParams({ order: id }); if (order.orderNumber) query.set('number', order.orderNumber); if (order.paymentMode) query.set('mode', order.paymentMode); if (typeof order.amountPaid === 'number') query.set('paid', String(order.amountPaid)); if (typeof order.amountDue === 'number') query.set('due', String(order.amountDue)); router.push(`/checkout/${state}?${query.toString()}`); };
  const verify = async (response: RazorpaySuccess, localOrderId: string): Promise<void> => { try { const result = await api.post<ApiEnvelope<{ verified: boolean; order?: Order }>>('/payments/razorpay/verify', { method: 'razorpay', payload: response }); if (!result.data.data.verified) throw new Error('We could not verify this payment.'); finish(result.data.data.order ?? { _id: localOrderId }, 'success'); } catch { setPaymentMessage('Your payment is being checked. View its current status in My Orders.'); router.push(`/checkout/success?order=${encodeURIComponent(localOrderId)}&pending=1`); } };
