...
export default function CheckoutPage(): ReactNode {
  ...
  const { register, handleSubmit, formState, watch, setValue } = useForm<CheckoutForm>({ resolver: zodResolver(checkoutPageSchema), defaultValues: { country: 'India', paymentMethod: 'razorpay', shippingMethod: 'standard' } });
  const onSubmit = (data: CheckoutForm): void => {
    ...
    checkout.mutate({ shippingAddress: address, billingAddress: address, paymentMethod: data.paymentMethod }, { onSuccess: (result) => router.push('/checkout/success?order=' + (result.order.id ?? result.order._id ?? result.payment.id)) });
  };
