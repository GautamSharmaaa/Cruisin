  ...
  return <>
    <header className="border-b border-border pb-8">
      ...
      <div className="mt-5 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3"><span className={order.paymentStatus === 'failed' ? 'border border-danger/50 bg-danger/10 px-4 py-2 text-xs uppercase tracking-[0.12em] text-text-primary' : 'border border-accent-gold/50 bg-accent-gold/10 px-4 py-2 text-xs uppercase tracking-[0.12em] text-accent-gold'}>{humanizeOrderStatus(displayStatus)}</span>{canCustomerCancel(order) ? <OrderCancellationDialog order={order} /> : null}</div>
      </div>
    </header>
