  ...
  return <>
    <header className="border-b border-border pb-8">
      ...
      <div className="mt-5 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3"><span className={order.paymentStatus === 'failed' || order.paymentStatus === 'cancelled' ? 'border border-danger/50 bg-danger/10 px-4 py-2 text-xs uppercase tracking-[0.12em] text-text-primary' : 'border border-accent-gold/50 bg-accent-gold/10 px-4 py-2 text-xs uppercase tracking-[0.12em] text-accent-gold'}>{humanizeOrderStatus(displayStatus)}</span>{canCustomerCancel(order) ? <OrderCancellationDialog order={order} /> : null}</div>
      </div>
    </header>
    ...
    <div className="mt-8 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <aside className="grid gap-6 lg:sticky lg:top-28">
        ...
        <section className="border border-border bg-background-elevated p-5 sm:p-6" aria-labelledby="shipping-heading"><div className="flex items-center gap-3"><Truck className="h-5 w-5 text-accent-gold" aria-hidden="true" /><h2 id="shipping-heading" className="font-display text-2xl">Delivery</h2></div><dl className="mt-4 grid gap-3 text-sm"><div className="flex justify-between gap-4"><dt className="text-text-muted">Method</dt><dd className="text-right text-text-primary">{humanizeOrderStatus(order.shippingMethod ?? 'standard')} delivery</dd></div><div className="flex justify-between gap-4"><dt className="text-text-muted">Fulfilment</dt><dd className="text-right text-text-primary">{humanizeOrderStatus(order.fulfillmentStatus ?? 'unfulfilled')}</dd></div></dl><Link href={`/account/orders/${id}/tracking`} className="mt-5 inline-flex min-h-11 w-full items-center justify-center border border-accent-gold px-4 text-xs uppercase tracking-[0.12em] text-accent-gold transition hover:bg-accent-gold hover:text-text-inverse">Track shipment</Link></section>
        <section className="border border-border bg-background-elevated p-5 sm:p-6" aria-labelledby="payment-heading"><div className="flex items-center gap-3"><CreditCard className="h-5 w-5 text-accent-gold" aria-hidden="true" /><h2 id="payment-heading" className="font-display text-2xl">Payment</h2></div><dl className="mt-4 grid gap-3 text-sm"><div className="flex justify-between gap-4"><dt className="text-text-muted">Mode</dt><dd className="text-right text-text-primary">{humanizeOrderStatus(order.paymentMode ?? order.paymentMethod ?? 'online')}</dd></div><div className="flex justify-between gap-4"><dt className="text-text-muted">Status</dt><dd className="text-right text-text-primary">{humanizeOrderStatus(order.paymentStatus)}</dd></div></dl>{order.paymentStatus === 'failed' ? <p role="alert" className="mt-5 border-l-2 border-danger pl-3 text-sm leading-6 text-text-secondary">The payment attempt failed and no successful charge is recorded for this order. Return to checkout to try again.</p> : null}{order.paymentStatus === 'cancelled' && order.cancellation?.reasonCode === 'payment_cancelled' ? <p role="status" className="mt-5 border-l-2 border-danger pl-3 text-sm leading-6 text-text-secondary">You closed the payment window. No payment was collected and this unpaid order was cancelled.</p> : null}{order.refunds?.length ? <div className="mt-5 border-t border-border pt-5"><p className="text-xs uppercase tracking-[0.12em] text-text-muted">Refund history</p><div className="mt-3 grid gap-3">{order.refunds.map((refund, index) => <div key={refund.providerRefundId ?? `${refund.amount}-${refund.createdAt}-${index}`} className="border-l-2 border-accent-gold pl-3 text-sm"><p className="text-text-primary">{formatPrice(refund.amount)} · {humanizeOrderStatus(refund.status)}</p>{refund.reason ? <p className="mt-1 leading-5 text-text-secondary">{refund.reason}</p> : null}{refund.createdAt ? <p className="mt-1 text-xs text-text-muted">{formatDate(refund.createdAt, true)}</p> : null}</div>)}</div></div> : null}</section>
      </aside>
    </div>
  </>;
};
