...
export function RefundDestinationPanel({ request }: { request: CustomerReturn }): ReactNode {
  ...
  if (request.refundDestination?.verificationStatus === 'verified' && !editingExisting) return <section className="mt-5 border border-success/40 bg-success/10 p-4 sm:p-5"><div className="flex gap-3"><ShieldCheck className="h-5 w-5 shrink-0 text-success" /><div className="min-w-0"><h3 className="font-display text-lg">{request.refundDestination.method === 'original_payment' ? 'Original payment refund ready' : 'Refund destination verified'}</h3><p className="mt-1 break-all text-sm text-text-secondary">{destinationDetails} · {request.refundDestination.registeredName ?? 'Identity verified'}</p>{submittedBy ? <p className="mt-1 text-xs text-text-muted">{submittedBy}</p> : null}<p className="mt-2 text-xs text-text-muted">{request.refundDestination.method === 'original_payment' ? `Cruisin can now issue ${formatPrice(request.productRefundAmount ?? 0)} through Razorpay to the same payment source. No bank, wallet, or UPI details are needed.` : `Cruisin admin can now initiate ${formatPrice(request.productRefundAmount ?? 0)}. You do not need to share OTPs or payment PINs.`}</p>{request.refundDestination.method !== 'original_payment' ? <Button variant="secondary" className="mt-3 w-full sm:w-auto" onClick={() => setEditingExisting(true)}>Change refund destination</Button> : null}</div></div></section>;
  ...
  return <section className="mt-5 border border-accent-gold/60 bg-accent-gold/5 p-4 sm:p-6" aria-labelledby={`refund-title-${request._id}`}><div className="flex gap-3"><Landmark className="mt-1 h-5 w-5 shrink-0 text-accent-gold" /><div><p className="text-xs uppercase tracking-[0.12em] text-accent-gold">Action required</p><h3 id={`refund-title-${request._id}`} className="mt-1 font-display text-xl">Choose where {formatPrice(request.productRefundAmount ?? 0)} should go</h3><p className="mt-2 text-sm leading-6 text-text-secondary">{available.includes('original_payment') ? 'This online payment can be returned through Razorpay to the original payment source.' : 'For this COD order, you or a Cruisin admin can select Cruisin Wallet or add your UPI ID. The admin initiates money movement only after verification.'}</p></div></div>
