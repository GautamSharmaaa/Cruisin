...
export function OrderReturnPanel({ order, returnWindow }: { order: Order; returnWindow?: ShipmentTracking['returnWindow'] }): ReactNode {
  const localOrderId = order.id ?? order._id ?? '';
  const delivered = order.orderStatus === 'delivered' || order.status === 'delivered';
  const eligible = Boolean(returnWindow?.eligible ?? delivered);
  ...
  return <section className="border border-border bg-background-elevated p-5 sm:p-6" aria-labelledby="returns-heading">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><div className="flex items-center gap-3"><RotateCcw className="h-5 w-5 text-accent-gold" aria-hidden="true" /><h2 id="returns-heading" className="font-display text-2xl">Returns & exchanges</h2></div>{returnWindow ? <><p className={`mt-2 text-sm ${returnWindow.eligible ? 'text-success' : 'text-text-secondary'}`}>{returnWindow.eligible ? `${returnWindow.daysRemaining} ${returnWindow.daysRemaining === 1 ? 'day' : 'days'} left to request a return or exchange` : 'The 5-day return and exchange window has ended.'}</p><p className="mt-1 text-xs text-text-muted">Window closes {new Intl.DateTimeFormat('en-IN', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(returnWindow.endsAt))}</p></> : <p className="mt-2 text-sm text-text-secondary">Choose an issue and upload photos for eligible items within 5 days of delivery.</p>}</div><Button type="button" variant="secondary" disabled={!eligible} onClick={() => { setFlow(null); setOpen(true); }}>{eligible ? 'Return or exchange' : delivered ? 'Window closed' : 'Available after delivery'}</Button></div>
