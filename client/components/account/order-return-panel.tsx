...
import { useCreateExchange, useCreateReturn, useExchangeOptions, useMyReturns, useMyExchanges, useUploadReturnEvidence, useVerifyExchangePayment, useVerifyReturnPayment, type ReturnEvidenceUpload } from '@/hooks/useReturns';
...
interface OrderReturnPanelProps {
  order: Order;
...
export function OrderReturnPanel({ order, returnWindow, trackingPending, trackingError }: OrderReturnPanelProps): ReactNode {
  const localOrderId = order.id ?? order._id ?? '';
  const eligibility = returnEligibility({ order, returnWindow, trackingPending, trackingError });
  const eligible = eligibility === 'eligible';
  const eligibilityCopy = {
    checking: [RETURN_EXCHANGE_COPY.checking, RETURN_EXCHANGE_COPY.checkingButton],
    unavailable: [RETURN_EXCHANGE_COPY.unavailable, RETURN_EXCHANGE_COPY.unavailableButton],
    before_delivery: [RETURN_EXCHANGE_COPY.beforeDelivery, RETURN_EXCHANGE_COPY.afterDeliveryButton],
    cancelled: [RETURN_EXCHANGE_COPY.cancelled, RETURN_EXCHANGE_COPY.cancelledButton],
    expired: [RETURN_EXCHANGE_COPY.expired, RETURN_EXCHANGE_COPY.expiredButton],
    eligible: ['', RETURN_EXCHANGE_COPY.requestButton],
  }[eligibility];
  ...
  useEffect(() => {
    ...
    const onDialogKeyDown = (event: KeyboardEvent): void => {
      ...
      const dialog = document.querySelector<HTMLElement>('[role="dialog"][aria-labelledby="return-dialog-title"]');
      const controls = Array.from(dialog?.querySelectorAll<HTMLElement>('button:not(:disabled), a[href], input:not(:disabled):not([type="file"]), select:not(:disabled), textarea:not(:disabled), [tabindex="0"]') ?? []).filter((element) => element.getClientRects().length > 0);
    ...
    return () => { document.removeEventListener('keydown', onDialogKeyDown); document.body.style.overflow = previousOverflow; opener?.focus(); };
  }, [open]);
  ...
  const orderExchanges = (exchanges.data ?? []).filter((request) => String(request.order) === localOrderId);
  ...
  return <section className="border border-border bg-background-elevated p-5 sm:p-6" aria-labelledby="returns-heading">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><div className="flex items-center gap-3"><RotateCcw className="h-5 w-5 text-accent-gold" aria-hidden="true" /><h2 id="returns-heading" className="font-display text-2xl">Returns & exchanges</h2></div><p className={`mt-2 text-sm ${eligible ? 'text-success' : 'text-text-secondary'}`} role="status">{eligible && returnWindow ? `${returnWindow.daysRemaining} ${returnWindow.daysRemaining === 1 ? 'day' : 'days'} left to request a return or exchange` : eligibilityCopy[0]}</p>{returnWindow && (eligible || eligibility === 'expired') ? <p className="mt-1 text-xs text-text-muted">Window closes {new Intl.DateTimeFormat('en-IN', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(returnWindow.endsAt))}</p> : null}</div><Button type="button" variant="secondary" disabled={!eligible} onClick={() => { setFlow(null); setOpen(true); }}>{eligibilityCopy[1]}</Button></div>
    ...
    <ExchangeRequestHistory requests={orderExchanges} />
  </section>;
}

interface ExchangeFlowProps { orderId: string; }

function ExchangeFlow({ orderId }: ExchangeFlowProps): ReactNode {
  ...
  const submit = async (): Promise<void> => {
    ...
    setMessage('');
    const storageKey = `cruisin:exchange-attempt:${orderId}:${original.variantId}:${requestedVariantId}:${quantity}`;
    let idempotencyKey = window.localStorage.getItem(storageKey);
    if (!idempotencyKey) { idempotencyKey = crypto.randomUUID(); window.localStorage.setItem(storageKey, idempotencyKey); }
    try {
      const session = await createExchange.mutateAsync({ orderId, variantId: original.variantId, requestedVariantId, quantity, idempotencyKey });
      if (session.request.handlingFeePaymentStatus === 'paid') { window.localStorage.removeItem(storageKey); setSubmitted(session.request.requestNumber); return; }
      if (!session.payment) throw new Error('A secure exchange payment session could not be created.');
      const key = process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID;
      if (!key) throw new Error('Online payments are not configured for this storefront.');
      await loadRazorpay();
      const razorpay = new window.Razorpay!({ key, amount: Math.round(session.payment.amount * 100), currency: 'INR', name: 'CRUISIN', description: `Exchange handling fee · ${session.request.requestNumber}`, order_id: session.payment.id, prefill: { name: user?.name, email: user?.email, contact: user?.phone }, handler: (response: RazorpaySuccess) => { void verifyExchange.mutateAsync({ requestId: session.request.id, payload: { ...response } }).then((request) => { window.localStorage.removeItem(storageKey); setSubmitted(request.requestNumber); }).catch((error: unknown) => setMessage(error instanceof Error ? error.message : 'Payment verification is pending.')); }, modal: { ondismiss: () => setMessage('Payment was not completed. Your exchange has not been submitted; you can retry safely.') }, theme: { color: '#b89b5e' } });
