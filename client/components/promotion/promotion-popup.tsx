...
export function PromotionPopup({ promotion, open, onClose }: PromotionPopupProps): ReactNode {
  ...
  return <Dialog.Root open={open} onOpenChange={(next) => { if (!next) dismiss(); }}><AnimatePresence>{open ? <Dialog.Portal forceMount>
    ...
    <Dialog.Content asChild aria-describedby="promotion-popup-description"><motion.section initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 24 }} transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }} className="promotion-popup-sheet fixed inset-x-0 bottom-0 z-[141] max-h-[min(92dvh,760px)] overflow-y-auto border-t border-accent-gold/50 bg-background-elevated px-6 pb-[max(24px,env(safe-area-inset-bottom))] pt-12 shadow-lg md:bottom-auto md:left-1/2 md:right-auto md:top-1/2 md:w-[calc(100vw-32px)] md:max-w-lg md:-translate-x-1/2 md:-translate-y-1/2 md:border">
      <Dialog.Close asChild><button type="button" aria-label="Close promotion" disabled={apply.isPending || success} className="absolute right-3 top-3 flex h-11 w-11 items-center justify-center text-text-secondary transition hover:text-text-primary disabled:opacity-40"><X size={20} /></button></Dialog.Close>
