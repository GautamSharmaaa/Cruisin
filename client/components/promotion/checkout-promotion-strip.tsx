...
export function CheckoutPromotionStrip({ disabled = false }: { disabled?: boolean }): ReactNode {
  ...
  const contents = <span className="flex items-center justify-center gap-3"><span className="hidden shrink-0 text-accent-gold md:inline-flex">{applied ? <Check size={16} /> : <Gift size={16} />}</span><span>{copy}</span></span>;
  return <div>{applied ? <div role="status" className="border-y border-accent-gold/30 bg-background-elevated px-5 py-5 text-center text-[11px] uppercase tracking-[0.14em] text-accent-gold md:border md:py-4 md:text-xs md:text-text-primary">{contents}</div> : <button type="button" onClick={() => apply.mutate()} disabled={disabled || apply.isPending} className="w-full border-y border-accent-gold/30 bg-background-elevated px-5 py-5 text-center text-[11px] uppercase tracking-[0.14em] text-accent-gold transition hover:border-accent-gold disabled:cursor-wait disabled:opacity-60 md:border md:py-4 md:text-xs md:text-text-primary">{contents}</button>}{message ? <p className="mt-2 text-center text-xs text-danger" aria-live="polite">{message}</p> : null}</div>;
}
