...
function ShopTheLook({ section, content, className }: { section: CmsSectionDto; content: Content; className: string; }): ReactNode {
  ...
  const firstProduct = ((section.products ?? []) as unknown[]).find((product): product is Product => typeof product === 'object' && product !== null && 'slug' in product);
  const configuredHref = asString(content, 'ctaLink');
  const href = safeHref(configuredHref && configuredHref !== '/shop' ? configuredHref : firstProduct ? '/product/' + firstProduct.slug : '/shop');
  const label = firstProduct?.title ?? section.title;
  return <section className={className + ' grid gap-px bg-border md:grid-cols-[1.2fr_0.8fr]'}><Link href={href} aria-label={'View ' + label} className="relative min-h-[620px] bg-background-elevated">{image ? <img src={image} alt={label} className="absolute inset-0 h-full w-full object-cover opacity-80 transition duration-700 hover:scale-[1.02] hover:opacity-100" /> : null}{hotspots.map((hotspot, index) => { const [hotspotLabel, x = '50', y = '50'] = hotspot.split('|'); return <span key={hotspot} className="absolute flex h-8 w-8 items-center justify-center rounded-full border border-background-primary bg-accent-gold text-xs text-text-inverse" style={{ left: x + '%', top: y + '%' }} aria-label={hotspotLabel}>{index + 1}</span>; })}</Link><div className="bg-background-primary p-8 lg:p-16"><p className="font-accent text-xs uppercase tracking-[0.18em] text-accent-gold">Shop The Look</p><h2 className="mt-5 font-display text-5xl">{section.title}</h2><p className="mt-5 text-text-secondary">{section.subtitle}</p><Link className="mt-8 inline-flex h-12 items-center border border-border px-7 text-xs uppercase tracking-[0.08em]" href={href}>{asString(content, 'ctaText', firstProduct ? 'View Product' : 'Explore Products')}</Link></div></section>;
}
