...
function CmsSectionRenderer({ section }: { section: CmsSectionDto }): ReactNode {
  ...
  if (type === 'product_carousel' || type === 'trending_now' || type === 'hot_drop') return <ProductRail section={section} content={content} className={visibility} />;
...
function VideoLanding({ section, content, className }: { section: CmsSectionDto; content: Content; className: string; }): ReactNode {
  return <section className={className + ' relative min-h-dvh overflow-hidden'}>
    <video src={asString(content, 'videoUrl')} poster={asString(content, 'posterImage')} autoPlay={asBool(content, 'autoplay', true)} muted={asBool(content, 'muted', true)} loop={asBool(content, 'loop', true)} playsInline className="absolute inset-0 h-full w-full object-cover opacity-75" />
