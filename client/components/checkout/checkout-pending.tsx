...
function PendingContent({ orderId }: CheckoutPendingProps): ReactNode {
  ...
  if (isOrderPaymentFailed(order.data)) {
    const wasCancelled = order.data.paymentStatus === 'cancelled';
    return <main className="px-6 py-32 text-center lg:px-20"><p className="font-mono text-xs uppercase tracking-[0.15em] text-accent-gold">{order.data.orderNumber ?? orderPath ?? COPY.checkout.pendingOrder}</p><h1 className="mt-4 font-display text-5xl">{wasCancelled ? 'Payment cancelled' : COPY.checkout.failure}</h1><p className="mx-auto mt-4 max-w-xl text-text-secondary">{wasCancelled ? 'No payment was collected and the unpaid order has been cancelled.' : COPY.checkout.failureBody}</p><div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><Link href={ROUTES.checkout} className={primaryLink}>{COPY.checkout.retry}</Link><Link href={ROUTES.orders} className={secondaryLink}>{COPY.checkout.myOrders}</Link></div></main>;
  }
