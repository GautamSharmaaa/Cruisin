...
export function OrderSummary({ shippingMethod = 'standard', shippingSettings, shippingAmountOverride, codFee = 0 }: OrderSummaryProps): ReactNode {
	...
	return (
		<aside className="border border-border bg-background-elevated/70 p-6 shadow-lg backdrop-blur-xl lg:sticky lg:top-28">
			...
			<section className="mt-6 border-y border-border py-5" aria-labelledby="checkout-coupon-heading">
				<p id="checkout-coupon-heading" className="mb-3 font-accent text-[10px] uppercase tracking-[0.18em] text-accent-gold">Coupon / promotion</p>
				<CouponInput />
			</section>
			<div className="mt-6 space-y-4 text-sm text-text-secondary">
				...
				{coupon ? <div className="flex items-start justify-between gap-4 text-success"><span className="min-w-0 break-words">Discount ({coupon})</span><span className="shrink-0 font-mono">{discount > 0 ? `-${formatPrice(discount)}` : freeShipping ? 'Free shipping' : formatPrice(0)}</span></div> : discount > 0 ? <div className="flex justify-between text-success"><span>Discount</span><span className="font-mono">-{formatPrice(discount)}</span></div> : null}
