	...
	return (
		<aside className="border border-border bg-background-elevated/70 p-6 shadow-lg backdrop-blur-xl lg:sticky lg:top-28">
			...
			<div className="mt-8 space-y-4 text-sm text-text-secondary">
				...
				<div className="flex justify-between border-t border-border pt-5 font-mono text-xl text-accent-gold"><span>{COPY.cart.total}</span><span>{formatPrice(subtotal + shipping + tax)}</span></div>
			</div>
		</aside>
	);
}
